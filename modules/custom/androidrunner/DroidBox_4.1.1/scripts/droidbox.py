...
tags = {
    ...
    0x2: 'TAINT_CONTACTS',
    0x4: 'TAINT_MIC',
    0x8: 'TAINT_PHONE_NUMBER',
    0x10: 'TAINT_LOCATION_GPS',
    0x20: 'TAINT_LOCATION_NET',
    0x40: 'TAINT_LOCATION_LAST',
    0x80: 'TAINT_CAMERA',
    0x100: 'TAINT_ACCELEROMETER',
    0x200: 'TAINT_SMS',
    0x400: 'TAINT_IMEI',
    0x800: 'TAINT_IMSI',
    0x1000: 'TAINT_ICCID',
    0x2000: 'TAINT_DEVICE_SN',
    0x4000: 'TAINT_ACCOUNT',
    0x8000: 'TAINT_BROWSER',
    0x10000: 'TAINT_OTHERDB',
    0x20000: 'TAINT_FILECONTENT',
    0x40000: 'TAINT_PACKAGE',
    0x80000: 'TAINT_CALL_LOG',
    0x100000: 'TAINT_EMAIL',
    0x200000: 'TAINT_CALENDAR',
    0x400000: 'TAINT_SETTINGS',
    }
...
class CountingThread(Thread):
    ...
    def run(self):
        ...
        while 0x1:
            ...
            counter = counter + 0x1
            if self.stop:
                sys.stdout.write("   \033[132m[%s] Collected %s sandbox logs\033[1m%s\r"
                                  % ('*', str(self.logs), ' ' * 25))
                sys.stdout.flush()
                break
            ...
            for i in zip.namelist():
                if i == 'AndroidManifest.xml':
                    try:
                        ...
                        xml[i] = \
                            minidom.parseString(AXMLPrinter(zip.read(i)).getBuff())
                    ...
                    for item in \
                        xml[i].getElementsByTagName('uses-permission'):
                    ...
                    for item in xml[i].getElementsByTagName('activity'):
                        ...
                        for child in item.getElementsByTagName('action'):
                            self.activities[activity]['actions'].append(str(
...
def main(argv):
    ...
    if ((len(argv) != 4) and (len(argv) != 5)):
        print 'Usage: droidbox.py filename.apk <duration in seconds> <device id> [output]'
        return -1
    apkName = argv[1]
    duration = int(argv[2])
    dev = argv[3]
    ...
    if os.path.isfile(apkName) == False:
        print 'File %s not found' % argv[0x1]
        sys.exit(0x1)
    ...
    ret = call(['monkeyrunner', dirname(abspath(__file__))
               + '/monkeyrunner.py', apkName, packageName,
               mainActivity], stderr=PIPE,
    ...
    adb = Popen(['adb', '-s', dev, 'logcat', 'DroidBox:W', 'dalvikvm:W',
                'ActivityManager:I'], stdin=subprocess.PIPE,
    ...
    while 1:
        try:
            ...
            if len(boxlog) > 0x1:
                try:
                    load = json.loads(decode(boxlog[0x1]))

            # DexClassLoader

                    if 'DexClassLoader' in load:
                        load['DexClassLoader']['type'] = 'dexload'
                        dexclass[time.time() - timeStamp] = \
                            load['DexClassLoader']
                        count.increaseCount()
            ...
            # service started
                    ...
                    if load.has_key('ServiceStart'):
                        ...
                        servicestart[time.time() - timeStamp] = \
                            load['ServiceStart']
                        count.increaseCount()
            ...
            # fdaccess
                    ...
                    if load.has_key('FdAccess'):
                        accessedfiles[load['FdAccess']['id']] = \
                            hexToStr(load['FdAccess']['path'])

            # file read or write

                    if load.has_key('FileRW'):
                        load['FileRW']['path'] = \
                            accessedfiles[load['FileRW']['id']]
                        if load['FileRW']['operation'] == 'write':
                            load['FileRW']['type'] = 'file write'
                        else:
                            load['FileRW']['type'] = 'file read'

                        fdaccess[time.time() - timeStamp] = \
                            load['FileRW']
                        count.increaseCount()
            ...
            # data leak log
                    ...
                    if load.has_key('DataLeak'):
                        ...
                        load['DataLeak']['tag'] = \
                            getTags(int(load['DataLeak']['tag'], 16))
                        ...
                        if load['DataLeak']['sink'] == 'Network':
                            ...
                            load['DataLeak']['path'] = \
                                accessedfiles[load['DataLeak']['id']]
            ...
            # phone call log
                    ...
                    if load.has_key('PhoneCall'):
                        ...
                        phonecalls[time.time() - timeStamp] = \
                            load['PhoneCall']
                        count.increaseCount()
            ...
            # crypto api usage log
                    ...
                    if load.has_key('CryptoUsage'):
                        ...
                        cryptousage[time.time() - timeStamp] = \
                            load['CryptoUsage']
                        count.increaseCount()
                except ValueError:
                    pass
    ...
    json.dump(output, open(json_out, "w"))
    sys.exit(0)
